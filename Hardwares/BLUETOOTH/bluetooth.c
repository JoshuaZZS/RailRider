...
//	while(__HAL_DMA_GET_COUNTER(&hdma_usart2_tx));  //检测DMA发送通道内还有没有数据
	if(Sendbuff)
	{
		memcpy( ( void * ) Usart2_Str.SentBuff, ( const void * )Sendbuff, l_val);
	}
	HAL_UART_Transmit_DMA(&huart2, Usart2_Str.SentBuff, l_val);
  return l_val;
}
...
uint8_t ParseBluetoothMessage(char *pInput ,uint16_t rcLens , BluetoothParse *blueParseMsg )
{
  ...
  if(*pInput == '#')//  #9,1,1,2,2,1*                                               //接收到数据的包头#
  {
    ...
    {
      /* 将读出的数据进行累加校验 */ 
      sum = (unsigned char)abs( ((int)atof(strtok(pInput+3, ",")) + 
...
void Bluetooth_RecvData(void)
{
  ...
  { 
    ParseBluetoothMessage( (char *)Usart2_Str.RecvBuff , Usart2_Str.RecvLens , &BluetoothParseMsg);   //进行数据解析
    memset (Usart2_Str.RecvBuff , 0,Usart2_Str.RecvLens);                 //缓存数据清除
    Usart2_Str.RecvLens = 0;                                                    //接收的数据长度清除
